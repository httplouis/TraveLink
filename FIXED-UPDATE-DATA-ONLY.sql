-- ═══════════════════════════════════════════════════════════════════════
-- FIXED UPDATE DATA ONLY - Matches your actual schema
...
-- Set Carlos Jayron A. Remiendo as VP (Comptroller/VP) - FIXED to match your schema
UPDATE users 
SET exec_type = 'vp'
WHERE (email = 'comptroller@mseuf.edu.ph' OR name ILIKE '%carlos%remiendo%')
  AND exec_type IS NULL;

-- Verification queries - FIXED to match your schema
...
-- Check executive users - FIXED to match your schema (using is_exec instead of is_executive)
SELECT 
  name,
  email,
  exec_type,
  ...
  is_president
FROM users 
WHERE exec_type IS NOT NULL OR is_exec = true OR is_vp = true OR is_president = true;
