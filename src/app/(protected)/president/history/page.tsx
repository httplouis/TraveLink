...
export default function PresidentHistoryPage() {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesFilters = Object.entries(activeFilters).every(([key, value]) => {
      ...
      if (key === 'status') {
        if (value === 'approved') return item.status === 'approved' || item.president_approved_at;
        if (value === 'rejected') return item.status === 'rejected' && item.rejection_stage === 'president';
      }
      return true;
    });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {filteredItems.length} {filteredItems.length === 1 ? 'request' : 'requests'} reviewed by President
          </p>
        </div>
        ...
        <div className="space-y-3">
          {filteredItems.map((item) => {
            ...
            const travelDate = item.travel_start_date
              ...
              : "—";
            const isApproved = item.status === 'approved' || item.president_approved_at;
            const isRejected = item.status === 'rejected' && item.rejection_stage === 'president';
            const actionDate = item.president_approved_at || item.rejected_at;
