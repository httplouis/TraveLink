...
export default function HRNotificationDropdown() {
  ...
  const loadNotifications = async () => {
    try {
      const [notificationsRes, inboxRes] = await Promise.all([
        ...
        fetch("/api/hr/inbox?limit=5", { cache: "no-store", credentials: "include" })
      ]);
      ...
      console.error("[HRNotificationDropdown] Failed to load notifications:", error);
      setNotifications([]);
      setUnreadCount(0);
    }
  };
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      channel = supabase
        .channel("hr-notifications-changes")
        .on(
          "postgres_changes",
          {
            event: "*",
      ...
      requestsChannel = supabase
        .channel("hr-inbox-requests-changes")
        .on(
          "postgres_changes",
          {
            event: "*",
          ...
          (payload: any) => {
            ...
            // Only react to changes that affect HR inbox
            if (newStatus === "pending_hr" || oldStatus === "pending_hr") {
              loadNotifications();
            }
          }
        )
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    if (notification.notification_type === "request_pending_approval" && notification.related_id) {
      router.prefetch(`/hr/inbox?view=${notification.related_id}`);
      router.push(`/hr/inbox?view=${notification.related_id}`);
    } 
    else if (notification.related_id && notification.related_type === "request") {
      router.prefetch(`/hr/request/submissions?view=${notification.related_id}`);
      router.push(`/hr/request/submissions?view=${notification.related_id}`);
    } else if (notification.action_url) {
      router.prefetch(notification.action_url);
      router.push(notification.action_url);
    } else {
      router.prefetch(`/hr/request/submissions`);
      router.push(`/hr/request/submissions`);
    }
  };
