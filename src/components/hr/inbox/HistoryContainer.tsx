...
export default function HRHistoryContainer() {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {filteredItems.length} {filteredItems.length === 1 ? 'request' : 'requests'} reviewed by HR
          </p>
        </div>
        ...
        <div className="space-y-3">
          {filteredItems.map((item) => {
            ...
            const travelDate = item.travel_start_date
              ...
              : "—";
            const isApproved = item.status === "approved" || item.status === "pending_exec";
            const isRejected = item.status === "rejected";
            const actionDate = item.hr_approved_at || item.rejected_at;
