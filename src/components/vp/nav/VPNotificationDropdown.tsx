...
export default function VPNotificationDropdown() {
  ...
  const loadNotifications = async () => {
    try {
      ...
      const inboxNotifications: Notification[] = inboxItems.map((item: any) => ({
        ...
        priority: "high",
        is_read: false,
        // Use hr_approved_at (when sent to VP) instead of created_at (when request was created)
        created_at: item.hr_approved_at || item.updated_at || item.created_at || new Date().toISOString(),
      }));
      ...
      console.error("[VPNotificationDropdown] Failed to load notifications:", error);
      setNotifications([]);
      setUnreadCount(0);
    }
  };
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    if (notification.notification_type === "request_pending_approval" && notification.related_id) {
      router.prefetch(`/vp/inbox?view=${notification.related_id}`);
      router.push(`/vp/inbox?view=${notification.related_id}`);
    } 
    else if (notification.related_id && notification.related_type === "request") {
      router.prefetch(`/vp/request/submissions?view=${notification.related_id}`);
      router.push(`/vp/request/submissions?view=${notification.related_id}`);
    } else if (notification.action_url) {
      router.prefetch(notification.action_url);
      router.push(notification.action_url);
    } else {
      router.prefetch(`/vp/request/submissions`);
      router.push(`/vp/request/submissions`);
    }
  };
