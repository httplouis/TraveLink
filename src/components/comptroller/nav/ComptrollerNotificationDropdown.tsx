...
export default function ComptrollerNotificationDropdown() {
  ...
  const loadNotifications = async () => {
    try {
      console.log("[ComptrollerNotificationDropdown] 🔍 Starting parallel fetches to /api/notifications and /api/comptroller/inbox");
      ...
      console.log("[ComptrollerNotificationDropdown] 📡 Inbox response:", {
        ok: inboxRes.ok,
      ...
      if (notificationsRes.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          console.log("[ComptrollerNotificationDropdown] ✅ Parsing notifications JSON...");
          notificationsData = await notificationsRes.json();
          console.log("[ComptrollerNotificationDropdown] ✅ Notifications JSON parsed:", { ok: notificationsData.ok, dataLength: notificationsData.data?.length });
        } else {
          console.warn("[ComptrollerNotificationDropdown] ❌ Notifications API returned non-JSON response");
          const errorText = await notificationsRes.text();
          console.error("[ComptrollerNotificationDropdown] ❌ Non-JSON response body:", errorText.substring(0, 500));
        }
      } else {
        console.warn("[ComptrollerNotificationDropdown] ❌ Notifications API not OK:", notificationsRes.status);
        const errorText = await notificationsRes.text();
        console.error("[ComptrollerNotificationDropdown] ❌ Error response body:", errorText.substring(0, 500));
      }
      
      if (inboxRes.ok) {
        const contentType = inboxRes.headers.get("content-type");
        if (contentType && contentType.includes("application/json")) {
          console.log("[ComptrollerNotificationDropdown] ✅ Parsing inbox JSON...");
          inboxData = await inboxRes.json();
          console.log("[ComptrollerNotificationDropdown] ✅ Inbox JSON parsed:", { isArray: Array.isArray(inboxData), dataLength: Array.isArray(inboxData) ? inboxData.length : 0 });
        } else {
          console.warn("[ComptrollerNotificationDropdown] ❌ Comptroller inbox API returned non-JSON response");
          const errorText = await inboxRes.text();
          console.error("[ComptrollerNotificationDropdown] ❌ Non-JSON response body:", errorText.substring(0, 500));
        }
      } else {
        console.warn("[ComptrollerNotificationDropdown] ❌ Comptroller inbox API not OK:", inboxRes.status);
        const errorText = await inboxRes.text();
        console.error("[ComptrollerNotificationDropdown] ❌ Error response body:", errorText.substring(0, 500));
      }
      ...
      const inboxItems = Array.isArray(inboxData) ? inboxData : [];
      ...
      const inboxNotifications: Notification[] = inboxItems.map((item: any) => ({
        ...
        priority: "high",
        is_read: false,
        // Use admin_processed_at (when sent to comptroller) instead of created_at (when request was created)
        created_at: item.admin_processed_at || item.updated_at || item.created_at || new Date().toISOString(),
      }));
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      channel = supabase
        .channel("comptroller-notifications-changes")
        .on(
          "postgres_changes",
          {
            event: "*",
      ...
      requestsChannel = supabase
        .channel("comptroller-inbox-requests-changes")
        .on(
          "postgres_changes",
          {
            event: "*",
          ...
          (payload: any) => {
            ...
            // Only react to changes that affect comptroller inbox
            if (newStatus === "pending_comptroller" || oldStatus === "pending_comptroller") {
              loadNotifications();
            }
          }
        )
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      if (!notificationId.startsWith('inbox-')) {
  ...
  const markAllAsRead = async () => {
    try {
      const unreadIds = notifications.filter(n => !n.is_read && !n.id.startsWith('inbox-')).map(n => n.id);
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    if (notification.notification_type === "request_pending_approval" && notification.related_id) {
      router.prefetch(`/comptroller/inbox?view=${notification.related_id}`);
      router.push(`/comptroller/inbox?view=${notification.related_id}`);
    } 
    else if (notification.related_id && notification.related_type === "request") {
      router.prefetch(`/comptroller/submissions?view=${notification.related_id}`);
      router.push(`/comptroller/submissions?view=${notification.related_id}`);
      ...
      <button
        onClick={() => {
          ...
          if (!isOpen) {
            loadNotifications();
            router.prefetch('/comptroller/inbox');
            router.prefetch('/comptroller/submissions');
          }
        }}
            ...
            {notifications.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  You'll see updates about budget reviews here
                </p>
              </div>
            ) : (
