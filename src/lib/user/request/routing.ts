...
export function firstReceiver({ requesterRole, vehicleMode }: { requesterRole: RequesterRole; vehicleMode: VehicleMode; reason: Reason; }): FirstReceiver {
...
export function fullApprovalPath({ requesterRole, vehicleMode }: { requesterRole: RequesterRole; vehicleMode: VehicleMode; }): string[] {
...
if (requesterRole === "head" && vehicleMode === "institutional") return ["TM", "COMPTROLLER", "HRD", "VP/COO", "TM(close-out)"];
if (requesterRole === "faculty" && vehicleMode !== "institutional") return ["DEPT_HEAD", "COMPTROLLER", "HRD", "VP/COO"];
