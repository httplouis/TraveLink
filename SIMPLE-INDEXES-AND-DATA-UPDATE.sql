-- ═══════════════════════════════════════════════════════════════════════
-- SIMPLE INDEXES AND DATA UPDATE - No Concurrent Operations
-- TraviLink v2.1 - Revolutionary Auto-Skip Logic
-- ═══════════════════════════════════════════════════════════════════════

-- Step 2: Create indexes (regular creation, not concurrent)
...
WHERE tablename IN ('requests', 'departments')
  AND (indexname LIKE '%smart%' OR indexname LIKE '%budget%' OR indexname LIKE '%exec%' OR indexname LIKE '%international%' OR indexname LIKE '%parent%')
